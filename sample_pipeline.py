...
def get_encodings_from_dataloader(query, split, model, dataloader):
    ...
    iterators, _ = dataloader.dataset.get_subset_iterators(iteratorspecs, cfg.vae.batch_size, torch.device(device))
...
def build_clfZ(attr):
    """
    从训练集中，经过VAE模型获取 正负样本的 隐式空间表示。
    用生成隐式空间 去训练一个 分类器。分类器的label为1/0，表示接受和拒绝
...
def main(args={}):
    ...
    #为两个属性分别训练自己的 接受/拒绝模型
    for attr in ['amp', 'tox']:
        clf_zspace = build_clfZ(attr) #训练分类器
...
if __name__ == "__main__":
    ...
    cfg._update_cfg()
    cfg._print(cfg)
    args.config_json = "output/amp/config_overrides.json"
    main(args)
